...
def _cli():
    parser = argparse.ArgumentParser(description="Manage Telegram invite rotation utilities.")
    parser.add_argument("action", choices=["rotate"], help="Action to perform")
    parser.add_argument("--expire-hours", type=float, default=48.0, dest="expire_hours")
    parser.add_argument("--count", type=int, default=INVITE_COUNT, dest="count")
    parser.add_argument("--members-per-link", type=int, default=INVITE_MEMBER_LIMIT, dest="members_per_link")
    parser.add_argument("--marker", default=INVITE_MARKER, dest="marker")
    parser.add_argument(
        "--skip-revoke", action="store_true", help="Do not revoke invites from the previous run"
    )
    parser.add_argument(
        "--skip-discord", action="store_true", help="Do not post the new invite batch to Discord"
    )
    parser.add_argument(
        "--state-file",
        default=str(INVITE_STATE_PATH),
        help="Override the path used to persist the latest invite batch",
    )

    args = parser.parse_args()

    if args.action == "rotate":
        links = rotate_invites_once(
            expire_days=args.expire_hours / 24.0,
            invite_count=args.count,
            members_per_link=args.members_per_link,
            marker=args.marker,
            revoke_previous=not args.skip_revoke,
            post_to_discord=not args.skip_discord,
            state_path=args.state_file,
        )
        for idx, link in enumerate(links, start=1):
            logging.info("Invite %d: %s", idx, link.get("invite_link"))
